...
import { aggEmpleado, aggEmpleados } from "../../redux/empleado/EmpleadosDucks";
import { useLocation } from "react-router-dom";

const ContratistaEmpleado = (props) => {
    const dispatch = useDispatch();
    const sessionstate = useSelector((state) => state.auth);
    const auth = useSelector(state => state.auth);
    const location = useLocation();

    const empleados = useSelector((store) => store.empleados.empleado);
    ...
    useEffect(() => {
        var id_empleado = location.pathname.split("/").pop();
        var obj = {
            estado: ["A", "I"],
            id: auth.data_user.id_perfil === 1 ? id_empleado : null,
        };
        if (obj.id !== undefined && obj.id !== "") {
            dispatch(aggEmpleado(obj));
        }
        // filtrarObras(obj);
        ...
        <Card>
            ...
            <Row gutter={[16, 16]}>
                <Col span={24}>
                    ...
                    >
                        <b style={{ fontSize: '20px' }}>EMPLEADO</b>
                    </div>
                </Col>
            </Row>
...
export default ContratistaEmpleado;
